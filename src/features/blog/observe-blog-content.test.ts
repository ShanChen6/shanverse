...
import { observeBlogContent } from "./observe-blog-content";
...
for (const { device, width, height } of [
  ...
  for (const withObserver of [true, false]) {
    test(`${device} ${withObserver ? "observer" : "geometry fallback"} counts only after real content enters the viewport`, () => {
      ...
      view.move(height - 1);
      view.update();
      assert.equal(counts, 1, "even a tall article counts when its first pixels become visible");
      view.move(100);
      view.update();
...
test("a background tab defers the view until the document is visible", () => {
...
test("cleanup prevents a queued observer callback from counting the previous slug", () => {
